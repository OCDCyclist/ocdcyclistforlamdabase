const {addAccessToken} = require('../src/utils/addAccessToken');
...
describe('addAccessToken basic tests', () =>{
    test('addAccessToken exists and can be called', () => {
        expect( typeof( addAccessToken ) ).toBe('function');
    });
    test('addAccessToken returns an empty object if called with no arguments', () => {
        const actual = addAccessToken();
        expect( typeof(actual) ).toBe('object');
        expect( Object.keys(actual).length ).toBe(0);
    });

    test('addAccessToken returns an empty object if called with a non object', () => {
        const actual = addAccessToken(123);
        expect( typeof(actual) ).toBe('object');
        expect( Object.keys(actual).length ).toBe(0);
    });
});

describe('addAccessToken functional tests for a well formed Strava object', () =>{
    test('addAccessToken sets the accesstoken property to a scalar string value', () => {
        const actual = addAccessToken( recentRideRaw, '1' );
        expect( typeof(actual) ).toBe('object');
        expect( typeof(actual.accesstoken) ).toBe('string');
        expect( actual.accesstoken ).toBe('1');
    });

    test('addAccessToken does nothing if the accesstoken property is a scalar number value', () => {
        const actual = addAccessToken( recentRideRaw, 1 );
        expect( typeof(actual) ).toBe('object');
        expect( 'accesstoken' in actual ).toBe(false);
    });

    test('addAccessToken does nothing if the accesstoken property already exists', () => {
        const adjustObj =  Object.assign( {}, recentRideRaw);
        adjustObj.accesstoken = '42';
        const actual = addAccessToken( adjustObj );
        expect( typeof(actual) ).toBe('object');
        expect( actual.accesstoken).toBe('42');
    });
});
