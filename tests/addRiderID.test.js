const {addRiderID} = require('../src/utils/addRiderID');
...
describe('addRiderID basic tests', () =>{
    test('addRiderID exists and can be called', () => {
        expect( typeof( addRiderID ) ).toBe('function');
    });
    test('addRiderID returns an empty object if called with no arguments', () => {
        const actual = addRiderID();
        expect( typeof(actual) ).toBe('object');
        expect( Object.keys(actual).length ).toBe(0);
    });

    test('addRiderID returns an empty object if called with a non object', () => {
        const actual = addRiderID(123);
        expect( typeof(actual) ).toBe('object');
        expect( Object.keys(actual).length ).toBe(0);
    });
});

describe('addRiderID functional tests for a well formed Strava object', () =>{
    test('addRiderID sets the RiderID property to a scalar string value', () => {
        const actual = addRiderID( recentRideRaw, '1' );
        expect( typeof(actual) ).toBe('object');
        expect( typeof(actual.RiderID) ).toBe('string');
        expect( actual.RiderID ).toBe('1');
    });

    test('addRiderID does nothing if the RiderID property is a scalar number value', () => {
        const actual = addRiderID( recentRideRaw, 1 );
        expect( typeof(actual) ).toBe('object');
        expect( 'RiderID' in actual ).toBe(false);
    });

    test('addRiderID does nothing if the RiderID property already exists', () => {
        const adjustObj =  Object.assign( {}, recentRideRaw);
        adjustObj.RiderID = '42';
        const actual = addRiderID( adjustObj );
        expect( typeof(actual) ).toBe('object');
        expect( actual.RiderID).toBe('42');
    });
});
